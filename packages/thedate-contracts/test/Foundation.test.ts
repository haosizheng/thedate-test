...
context("Foundation", () => {
  ...
  describe("Claim funds to non members", async () => {
    ...
    it("Claim funds to non members for ETH", async () => {
      await expect(foundationContract.release(user3.address)).to.be.revertedWith(
        "PaymentSplitter: account has no shares",
      );
    });

    it("Claim funds to non members for ERC20", async () => {
      await expect(foundationContract.releaseERC20(user4.address, mockERC20Contract.address)).to.be.revertedWith(
        "Foundation: account has no shares",
      );
    });

    it("Claim funds to non members for ERC721", async () => {
      await expect(foundationContract.releaseERC721(user4.address, mockERC721Contract.address, 0)).to.be.revertedWith(
        "Foundation: account has no shares",
      );
    });

    it("Claim funds to non members for ERC1155", async () => {
      await expect(foundationContract.releaseERC1155(user4.address, mockERC1155Contract.address, 1, 1)).to.be.revertedWith(
        "Foundation: account has no shares",
      );
    });
  });
  ...
  describe("Verify valid tokens", async () => {
    ...
    it("Exception case to release ERC20 for Empty Contract", async () => {      
      await expect(
        foundationContract.releaseERC20(foundationUsers[0].address, mockEmptyContract.address),
    ...
    it("Exception case to release ERC721 for Empty Contract", async () => {      
      await expect(
        foundationContract.releaseERC721(foundationUsers[0].address, mockEmptyContract.address, 1),
      ).to.be.revertedWith("Foundation: not a valid ERC721");      
    });

    it("Exception case to release ERC1155 for Empty Contract", async () => {      
      await expect(
        foundationContract.releaseERC1155(foundationUsers[0].address, mockEmptyContract.address, 1, 1),
      ).to.be.revertedWith("Foundation: not a valid ERC1155");      
    });

    it("Exception case to release ERC20 for ERC165-only Contract ", async () => {      
      await expect(
        foundationContract.releaseERC20(foundationUsers[0].address, mockERC165Contract.address),
      ).to.be.revertedWith("Foundation: not a valid ERC20");
    });

    it("Exception case to release ERC721 for ERC165-only Contract ", async () => {    
      await expect(
        foundationContract.releaseERC721(foundationUsers[0].address, mockERC165Contract.address, 1),
      ).to.be.revertedWith("Foundation: not a valid ERC721");      
    });

    it("Exception case to release ERC1155 for ERC165-only Contract ", async () => {    
      await expect(
        foundationContract.releaseERC1155(foundationUsers[0].address, mockERC165Contract.address, 1, 1),
      ).to.be.revertedWith("Foundation: not a valid ERC1155");      
    });
  ...
  describe("NFT transfers correctly", async () => {
    ...
    it("No tokenId to release for ERC721. ", async () => {
      const tokenId = 101;
      await expect(
        foundationContract.releaseERC721(foundationUsers[0].address, mockERC721Contract.address, tokenId),
    ...
    it("Not enough amount of tokenId to release for ERC1155. ", async () => {
      ...
      // No fund to release
      await expect(
        foundationContract.releaseERC1155(foundationUsers[0].address, mockERC1155Contract.address, tokenId, 1),
    ...
    it("ERC721 transfers correctly", async () => {
      ...
      await expect(mockERC721Contract.mint(foundationContract.address, tokenId))
        .to.emit(mockERC721Contract, "Transfer").withArgs(ethers.constants.AddressZero, foundationContract.address, tokenId);
      ...
      await expect(foundationContract.releaseERC721(foundationUsers[0].address, mockERC721Contract.address, tokenId))
        .to.emit(mockERC721Contract, "Transfer").withArgs(foundationContract.address, foundationUsers[0].address, tokenId)
        .to.emit(foundationContract, "ERC721PaymentReleased").withArgs(foundationUsers[0].address, mockERC721Contract.address, tokenId);
    });
    ...
    it("ERC1155 transfers correctly", async () => {
      ...
      await expect(mockERC1155Contract.mint(foundationContract.address, tokenId, 1))
        .to.emit(mockERC1155Contract, "TransferSingle").withArgs(deployer.address, ethers.constants.AddressZero, foundationContract.address, tokenId, 1);

      await expect(foundationContract.releaseERC1155(foundationUsers[0].address, mockERC1155Contract.address, tokenId, 1))
        .to.emit(mockERC1155Contract, "TransferSingle").withArgs(foundationContract.address, foundationContract.address, foundationUsers[0].address, tokenId, 1)
        .to.emit(foundationContract, "ERC1155PaymentReleased").withArgs(foundationUsers[0].address, mockERC1155Contract.address, tokenId, 1);
    });
  });
  ...
  describe("Anyone can trigger the release", async () => {
    ...
    it("Anyone can release the funds for members for ETH.", async () => {
      ...
      await expect(() =>
        deployer.sendTransaction({ to: foundationContract.address, value: amount }),
      ).to.changeEtherBalance(foundationContract, amount);

      // Release funds to foundationUsers[0] invoked by user3
      await expect(() => foundationContract.connect(user3).release(foundationUsers[0].address)).to.changeEtherBalance(
        foundationUsers[0],
        portionAmount,
      );
    ...
    it("Anyone can release the funds for members for ERC20.", async () => {
      ...
      // Transfer fund to foundation.
      await expect(() => mockERC20Contract.transfer(foundationContract.address, amount)).to.changeTokenBalance(
        mockERC20Contract,
        foundationContract,
        amount,
      );
      ...
      // Release funds to foundationUsers[0] invoked by user3
      await expect(() =>
        foundationContract.connect(user3).releaseERC20(foundationUsers[0].address, mockERC20Contract.address),
      ).to.changeTokenBalance(mockERC20Contract, foundationUsers[0], portionAmount);

      // Released
      expect(await foundationContract.releasedERC20(foundationUsers[0].address, mockERC20Contract.address)).to.eq(
        portionAmount,
      );
    });
  ...
  describe("Payment splits correctly", async () => {
    ...
    it("No fund to release for ETH. ", async () => {
      ...
      // No fund to release
      await expect(foundationContract.release(foundationUsers[0].address)).to.be.revertedWith(
    ...
    it("No fund to release for ERC20. ", async () => {
      ...
      // No fund to release
      await expect(
        foundationContract.releaseERC20(foundationUsers[0].address, mockERC20Contract.address),
    ...
    it("Should split ETH correctly", async () => {
      ...
      for (const i in foundationMembers) {
        ...
        // Release
        await expect(() => foundationContract.release(foundationUsers[i].address)).to.changeEtherBalance(
          foundationUsers[i],
          portionAmount,
        );
    ...
    it("Should split ERC20 correctly", async () => {
      ...
      for (const i in foundationMembers) {
        ...
        // Release
        await expect(() =>
          foundationContract.releaseERC20(foundationUsers[i].address, mockERC20Contract.address),
        ).to.changeTokenBalance(mockERC20Contract, foundationUsers[i], portionAmount);

        // Check released
        expect(await foundationContract.releasedERC20(foundationUsers[i].address, mockERC20Contract.address)).to.eq(
          portionAmount,
        );
      }
    ...
    it("Waterfall the payment recursively", async () => {
      ...
      const foundationContract2 = await (
        await new Foundation__factory(deployer).deploy(foundationMembers2, foundationShares2)
      ).deployed();

      const amount = ethers.utils.parseEther("1");
      await expect(() =>
        deployer.sendTransaction({ to: foundationContract2.address, value: amount }),
      ...
      await expect(() => foundationContract2.release(foundationContract.address)).to.changeEtherBalance(
        foundationContract,
        portionAmountOfContract,
      );

      const portionAmountOfUser1 = portionAmountOfContract
        .mul(foundationShares[0])
        .div(foundationShares.reduce((sum, current) => sum + current, 0));
      await expect(() => foundationContract.release(user1.address)).to.changeEtherBalance(user1, portionAmountOfUser1);
    });
  });
});
