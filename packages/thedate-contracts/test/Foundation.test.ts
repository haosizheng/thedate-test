...
context("Foundation contract", () => {
  ...
  describe("Anyone can trigger the release", async () => {
    ...
    it("Anyone can release the funds for members for ETH.", async () => {
      ...
      await expect(await deployer.sendTransaction({ to: foundationContract.address, value: amount }))
        .to.changeEtherBalance(foundationContract, amount);

      // Release funds to foundationUsers[0] invoked by user3
      await expect(await foundationContract.connect(user3).release(foundationUsers[0].address))
  ...
  describe("Payment splits correctly", async () => {
    ...
    it("Should split ETH correctly", async () => {
      ...
      for (const i in foundationMembers) {
        ...
        // Release
        await expect(await foundationContract.release(foundationUsers[i].address))
    ...
    it("Waterfall the payment recursively", async () => {
      ...
      const amount = ethers.utils.parseEther("1");
      await expect(await deployer.sendTransaction({ to: foundationContract2.address, value: amount }))
      ...
      await expect(await foundationContract2.release(foundationContract.address))
      ...
      await expect(await foundationContract.release(user1.address))
