...
import {
  TheFoundation,
  TheFoundation__factory,
  TheNote,
  TheNote__factory,
  TheNoteProofOfBelief,
  TheNoteProofOfBelief__factory,
} from "../typechain";
...
context("TheNote", () => {
  ...
  beforeEach(async () => {
    ...
    mainContract = await (await new TheNote__factory(deployer).deploy(foundationContract.address)).deployed();

    // Deploy the pob contract
    pobContract = await (
      await new TheNoteProofOfBelief__factory(deployer).deploy(foundationContract.address, mainContract.address)
    ).deployed();
  });
