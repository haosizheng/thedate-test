...
import { MockFoundation, MockFoundation__factory, MockERC20, MockERC20__factory } from "../typechain";
...
context("Foundation", () => {
  let foundationContract: MockFoundation;
  ...
  beforeEach(async () => {
    ...
    foundationContract = await (
      await new MockFoundation__factory(deployer).deploy(foundationMembers, foundationShares)
    ).deployed();
  ...
  describe("Payment Splits", async () => {
    ...
    it("Exception case to release for non-ERC20. ", async () => {
      ...
      // No fund to release
      await expect(
        foundationContract.releaseERC20(foundationUsers[0].address, foundationContract.address),
    ...
    it("Waterfall the payment recursively", async () => {
      ...
      const foundationContract2 = await (
        await new MockFoundation__factory(deployer).deploy(foundationMembers2, foundationShares2)
      ).deployed();
    ...
    it("Should split Ethers correctly", async () => {
    ...
    it("Should split MockERC20 correctly", async () => {
      // Send some fund to foundation contract
      const amount = ethers.utils.parseEther("1");
      await expect(() => mockERC20Contract.transfer(foundationContract.address, amount)).to.changeTokenBalance(
        mockERC20Contract,
