...
import { TheFoundation, TheFoundation__factory, TheNote, TheNote__factory, TheNoteProofOfBelief, 
  TheNoteProofOfBelief__factory } from "../typechain";
...
describe("TheNote Test", () => {
  let foundationContract: TheFoundation;
  let mainContract: TheNote;
  let pobContract: TheNoteProofOfBelief;
  ...
  beforeEach(async () => {
    ...
    mainContract = await (await (new TheNote__factory(deployer)).deploy(
      foundationContract.address)).deployed();

    // Deploy the main contract
    pobContract = await (await (new TheNoteProofOfBelief__factory(deployer)).deploy(
        foundationContract.address, mainContract.address)).deployed();
  });

  describe("TheNote - Royalty", async () => {
    it("Royalty", async () => {
  ...
  describe("TheNote - PayToMint", async () => {
    it("PayToMint", async () => {
      await mainContract.setFixedPrice(ethers.utils.parseEther("0.1"));
      await pobContract.setFixedPrice(ethers.utils.parseEther("0.01"));
      const userNote = "I love you.";

      // Exception case that the bid is lower than fixed price
      await expect(mainContract.connect(user1).payToMint(userNote, {value: ethers.utils.parseEther("0.01")}))
      ...
      await expect(mainContract.connect(user1).payToMint(userNote, {value: ethers.utils.parseEther("0.1")}))
        .emit(mainContract, "ArtworkMinted").withArgs(noteId);

      // Exception case that the bid is lower than fixed price of pob
      await expect(pobContract.connect(user1).payToMint(noteId, {value: ethers.utils.parseEther("0.001")}))
        .to.be.revertedWith("Should pay >= fixed price");
      ...
      await expect(pobContract.connect(user1).payToMint (noteId, {value: ethers.utils.parseEther("0.01")}))
        .to.emit(pobContract, "Transfer").withArgs(ethers.constants.AddressZero, user1.address, pobId1);
      ...
      await expect(pobContract.connect(user2).payToMint(noteId, {value: ethers.utils.parseEther("0.01")}))
        .to.emit(pobContract, "Transfer").withArgs(ethers.constants.AddressZero, user2.address, pobId2);
